...
export class NotificationsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket): Promise<void> {
    ...
    try {
      ...
      client.emit('connected', { status: 'connected', userId, timestamp: new Date().toISOString() });
  ...
  async handleSubscribe(@ConnectedSocket() client: Socket, @MessageBody() data: { types: NotificationType[]; userId?: string }) {
    const connectedClient = this.connectedClients.get(client.id);
    if (!connectedClient) {
    ...
    client.emit('subscribed', { types, userId: targetUserId, timestamp: new Date().toISOString() });
  }

  @SubscribeMessage('unsubscribe')
  async handleUnsubscribe(@ConnectedSocket() client: Socket, @MessageBody() data: { types: NotificationType[] }) {
    const connectedClient = this.connectedClients.get(client.id);
    if (!connectedClient) {
      client.emit('error', { message: 'Client not registered' });
      return;
    ...
    client.emit('unsubscribed', { types, userId: connectedClient.userId, timestamp: new Date().toISOString() });
  }

  @SubscribeMessage('get_notifications')
  async handleGetNotifications(@ConnectedSocket() client: Socket, @MessageBody() data: { limit?: number; offset?: number; unreadOnly?: boolean }) {
    const connectedClient = this.connectedClients.get(client.id);
    if (!connectedClient) {
      client.emit('error', { message: 'Client not registered' });
      return;
    ...
    try {
      const notifications = await this.notificationsService.getUserNotifications(connectedClient.userId, data.limit || 50, data.offset || 0, data.unreadOnly || false);
      client.emit('notifications', { notifications, userId: connectedClient.userId, timestamp: new Date().toISOString() });
    } catch (error) {
  ...
  async handleMarkAsRead(@ConnectedSocket() client: Socket, @MessageBody() data: { notificationId: string }) {
    ...
    try {
      await this.notificationsService.markNotificationAsRead(connectedClient.userId, data.notificationId);
      const unreadCount = await this.notificationsService.getUnreadCount(connectedClient.userId);
      client.emit('notification_read', { notificationId: data.notificationId, unreadCount, timestamp: new Date().toISOString() });
    } catch (error) {
  ...
  async handleMarkAllAsRead(@ConnectedSocket() client: Socket) {
    ...
    try {
      ...
      client.emit('all_notifications_read', { unreadCount: 0, timestamp: new Date().toISOString() });
    } catch (error) {
  ...
  async handleUpdatePreferences(@ConnectedSocket() client: Socket, @MessageBody() data: Partial<any>) {
    const connectedClient = this.connectedClients.get(client.id);
    if (!connectedClient) {
      client.emit('error', { message: 'Client not registered' });
      return;
    ...
    try {
      const updatedPreferences = await this.notificationsService.updateNotificationPreferences(connectedClient.userId, data);
      client.emit('preferences_updated', { preferences: updatedPreferences, timestamp: new Date().toISOString() });
    } catch (error) {
  ...
  async handleGetPreferences(@ConnectedSocket() client: Socket) {
    ...
    try {
      const preferences = await this.notificationsService.getUserNotificationPreferences(connectedClient.userId);
      client.emit('preferences', { preferences, timestamp: new Date().toISOString() });
    } catch (error) {
  ...
  async sendSystemAnnouncement(title: string, message: string, data?: any, priority: 'low' | 'medium' | 'high' | 'urgent' = 'medium') {
